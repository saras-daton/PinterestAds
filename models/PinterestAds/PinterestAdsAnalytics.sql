...
{{set_table_name('%pinterest%ads_analytics')}}    
{% endset %}  
...
{% for i in results_list %}
    ...
    From (
        ...
        DENSE_RANK() OVER (PARTITION BY AD_ID,DATE,CONVERSION_REPORT_TIME order by a.{{daton_batch_runtime()}} desc) row_num
        from {{i}} a  
            {% if is_incremental() %}
