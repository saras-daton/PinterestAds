{% if var('PinterestAdGroupsAnalytics') %}
{{ config( enabled = True ) }}
{% else %}
...
{{set_table_name('%pinterest%ad_groups_analytics')}}    
{% endset %}  
...
{% for i in results_list %}
    ...
    From (
        ...
        DENSE_RANK() OVER (PARTITION BY AD_GROUP_ID,DATE,CONVERSION_REPORT_TIME order by a.{{daton_batch_runtime()}} desc) row_num
        from {{i}} a  
            {% if is_incremental() %}
